...
`pnpm test:e2e` runs the web Playwright project. Its smoke test starts the development server locally and the already-built production server in CI and confirms the root page identifies GŁĘBIA. Install Chromium with `pnpm exec playwright install chromium` before the first local run. CI installs Chromium with its operating-system dependencies.
